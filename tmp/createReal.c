/* createReal.c - createReal, newpid */
...
/*------------------------------------------------------------------------
 *  createReal  -  createReal a process to start running a procedure
 *------------------------------------------------------------------------
 */
...
/* emulate 386 "pushal" instruction */
	...
	*pushsp = pptr->pesp = (unsigned long)saddr;
	...
	restore(ps);
        ...
        queue_list[queue_index].pid=pid;
        queue_list[queue_index].counter=priority;          
     queue_list[queue_index].goodness=priority+queue_list[queue_index].counter;
        queue_index++;
